...
const Jeans = () => {
  const [jeans, setJeans] = useState([]); // Holds the fetched jeans products
  ...
  // Fetch jeans products from Firestore
  useEffect(() => {
    const fetchJeans = async () => {
      try {
        const productsCollection = collection(db, 'products');
        const q = query(productsCollection, where('category', '==', 'jeans'));

        const productsSnapshot = await getDocs(q);
        const jeansList = productsSnapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
        }));

        console.log('Fetched jeans products:', jeansList); // Debug fetched data
        setJeans(jeansList); // Set fetched jeans products
