...
const Fragrances = () => {
  const [fragrances, setFragrances] = useState([]); // Holds the fetched fragrances
  ...
  // Fetch fragrances from Firestore
  useEffect(() => {
    const fetchFragrances = async () => {
      try {
        const productsCollection = collection(db, 'products');
        const q = query(productsCollection, where('category', '==', 'fragrances'));

        const productsSnapshot = await getDocs(q);
        const fragrancesList = productsSnapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
        }));

        console.log('Fetched fragrances:', fragrancesList); // Debug fetched data
        setFragrances(fragrancesList); // Set fetched fragrances
