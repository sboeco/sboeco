...
const DataDevices = () => {
  const [dataDevices, setDataDevices] = useState([]); // Holds the fetched data devices
  ...
  // Fetch data devices from Firestore
  useEffect(() => {
    const fetchDataDevices = async () => {
      try {
        const productsCollection = collection(db, 'products');
        const q = query(productsCollection, where('category', '==', 'datadevices'));

        const productsSnapshot = await getDocs(q);
        const dataDevicesList = productsSnapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
        }));

        console.log('Fetched data devices:', dataDevicesList); // Debug fetched data
        setDataDevices(dataDevicesList); // Set fetched data devices
