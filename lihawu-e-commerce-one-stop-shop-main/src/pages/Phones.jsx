...
const Phones = () => {
  const [phones, setPhones] = useState([]); // Holds the fetched phones products
  ...
  // Fetch phones products from Firestore
  useEffect(() => {
    const fetchPhones = async () => {
      try {
        const productsCollection = collection(db, 'products');
        const q = query(productsCollection, where('category', '==', 'phones'));

        const productsSnapshot = await getDocs(q);
        const phonesList = productsSnapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
        }));

        console.log('Fetched phones products:', phonesList); // Debug fetched data
        setPhones(phonesList); // Set fetched phones products
