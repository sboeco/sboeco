...
const CellPhone = () => {
  const [cellPhones, setCellPhones] = useState([]); // Holds the fetched cellphones
  ...
  // Fetch cellphones from Firestore
  useEffect(() => {
    const fetchCellPhones = async () => {
      try {
        const productsCollection = collection(db, 'products');
        const q = query(productsCollection, where('category', '==', 'cellphones'));

        const productsSnapshot = await getDocs(q);
        const cellPhonesList = productsSnapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
        }));

        console.log('Fetched cellphones:', cellPhonesList); // Debug fetched data
        setCellPhones(cellPhonesList); // Set fetched cellphones
