...
const Skincare = () => {
  const [skincare, setSkincare] = useState([]); // Holds the fetched skincare products
  ...
  // Fetch skincare products from Firestore
  useEffect(() => {
    const fetchSkincare = async () => {
      try {
        const productsCollection = collection(db, 'products');
        const q = query(productsCollection, where('category', '==', 'skincare'));

        const productsSnapshot = await getDocs(q);
        const skincareList = productsSnapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
        }));

        console.log('Fetched skincare products:', skincareList); // Debug fetched data
        setSkincare(skincareList); // Set fetched skincare products
