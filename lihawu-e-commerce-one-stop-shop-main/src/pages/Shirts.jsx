...
const Shirt = () => {
  const [shirts, setShirts] = useState([]); // Holds the fetched shirts products
  ...
  // Fetch shirts products from Firestore
  useEffect(() => {
    const fetchShirts = async () => {
      try {
        const productsCollection = collection(db, 'products');
        const q = query(productsCollection, where('category', '==', 'shirts'));

        const productsSnapshot = await getDocs(q);
        const shirtsList = productsSnapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
        }));

        console.log('Fetched shirts products:', shirtsList); // Debug fetched data
        setShirts(shirtsList); // Set fetched shirts products
