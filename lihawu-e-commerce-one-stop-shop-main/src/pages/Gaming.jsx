...
const Gaming = () => {
  const [gaming, setGaming] = useState([]); // Holds the fetched gaming products
  ...
  // Fetch gaming products from Firestore
  useEffect(() => {
    const fetchGaming = async () => {
      try {
        const productsCollection = collection(db, 'products');
        const q = query(productsCollection, where('category', '==', 'gaming'));

        const productsSnapshot = await getDocs(q);
        const gamingList = productsSnapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
        }));

        console.log('Fetched gaming products:', gamingList); // Debug fetched data
        setGaming(gamingList); // Set fetched gaming products
